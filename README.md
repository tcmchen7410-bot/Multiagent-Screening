...
The multi-agent framework is built based on the OpenAI SDK framework. We have made public the script for the multi-agent interaction with ChatGpt as well as all the statistical analysis scripts. Our dataset consists of 300 citations (```dataset``` > ```sample_size.csv```) from the pretesting stage. Once the paper is officially accepted, we will make the complete dataset publicly available.
